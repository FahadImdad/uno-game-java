...
public class ChallengeOverlay extends WndInterface implements TurnDecisionOverlayInterface {
    ...
    private final List<Button> buttonList; // List of buttons that can be used in the overlay. Includes a Challenge and Decline button.

    private TurnActionFactory.TurnDecisionAction currentAction; // Reference to the TurnAction that triggered the display of this overlay.

    private final Player playerReference; // Reference to the player to be used for card selection when stacking is allowed.

    private final boolean allowStacking; // When true via the RuleSet the player's cards can be stacked if possible.
    ...
    //
    //      bounds: The bounds of the entire game area. The buttons are offset from the centre.
    ...
    @Override
    public void update(int deltaTime) { //   Not used.
    ...
    //
    //      currentAction: The TurnAction used to make this overlay appear.
