...
public class PostGameInterface extends WndInterface {
    ...
    private final List<Button> buttonList; // List of buttons visible on the interface.

    private final List<Player> players;// Reference to the players that are being shown for scores.

    private final RuleSet ruleSet;// Reference to the Ruleset to check for score limits.

    private final GamePanel gamePanel; //  Reference to the GamePanel for exiting the interface.

    private final List<String> playerStrings; // Strings to cache the String generation for the player list.

    private String roundWinnerStr; // Winner for this round, and possibly the entire match.

    private boolean scoreLimitReached; // When true, the score limit has been reached showing an extra message.

    private String scoreLimitStr; // A String showing the score limit rule.
     ...
     *
     * actionID The actionID for a button that was pressed.
     */
